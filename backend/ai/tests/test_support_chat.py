...
def test_support_chat_creates_ticket_for_technical_issue(authenticated_client, user, monkeypatch):
    ...
    notification = InAppNotification.objects.get(
        user=user, notification_type="support_ticket_created"
    )
...
@pytest.mark.django_db
def test_support_chat_does_not_create_ticket_for_general_question(
    authenticated_client, user, monkeypatch
):
    monkeypatch.setattr(
        "ai.views_support.complete", lambda **kwargs: "Here is how the platform works."
    )
