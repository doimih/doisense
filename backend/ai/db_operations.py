    ...
    try:
        with transaction.atomic():
            for operation in operations:
                ...
                operation_result = {"type": op_type, "status": "pending", "message": ""}
                ...
                try:
                    ...
                    results["errors"].append(
                        {"operation": op_type, "error": str(e), "data_sample": str(op_data)[:100]}
                    )
        ...
        results["errors"].append({"type": "transaction_error", "error": str(e)})
        logger.error(f"Transaction error for user {user.id}: {e}", exc_info=True)
        results["status"] = "transaction_failed"

    except Exception as e:
        results["errors"].append({"type": "unknown_error", "error": str(e)})
...
def _save_emotional_tags(user, data: Dict[str, Any]) -> None:
    ...
    profile.save(
        update_fields=["last_dominant_emotion", "secondary_emotions", "last_emotion_updated"]
    )
...
def _save_wellness_metrics(user, data: Dict[str, Any]) -> None:
    ...
    WellnessMetric.objects.create(
        user=user, stress_score=stress, energy_score=energy, motivation_score=motivation
    )
...
def process_db_saves_with_retry(
    user, save_request: Dict[str, Any], max_retries: int = 3
) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                # Last attempt failed
                logger.error(f"Failed to save after {max_retries} attempts for user {user.id}: {e}")
                return {
                    "timestamp": datetime.now().isoformat(),
                    "status": "failed",
                    "error": str(e),
            ...
            wait_time = 2**attempt
            logger.info(
                f"Retrying save for user {user.id} after {wait_time}s (attempt {attempt + 1}/{max_retries})"
            )
